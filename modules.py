class Module:
    ...
    def decode(self, input_bytes, m, n, l=None, is_ntt=False):
        ...
        chunk_length = 32*l
        byte_chunks = [input_bytes[i:i+chunk_length]
    ...
    class Matrix:
        ...
        def __matmul__(self, other):
            ...
            new_elements = [[sum(a*b for a, b in zip(A_row, B_col))
                             for B_col in other.transpose().rows] for A_row in self.rows]
            return self.parent(new_elements)
        ...
        def __repr__(self):
            ...
            info = ']\n['.join([', '.join(
                [f'{str(x):>{max_col_width[i]}}' for i, x in enumerate(r)]) for r in self.rows])
            return f"[{info}]"
