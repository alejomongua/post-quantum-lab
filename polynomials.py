...
class PolynomialRing:
    ...
    class Polynomial:
        ...
        def encode(self, l=None):
            ...
            bit_string = ''.join(format(c, f'0{l}b')[
                                 ::-1] for c in self.coeffs)
        ...
        def compress(self, d):
            ...
            self.coeffs = [round_up(compress_float * c) %
                           compress_mod for c in self.coeffs]
            return self
        ...
        def __add__(self, other):
            if isinstance(other, PolynomialRing.Polynomial):
                ...
                new_coeffs = [self.add_mod_q(x, y) for x, y in zip(
                    self.coeffs, other.coeffs)]
        ...
        def __sub__(self, other):
            if isinstance(other, PolynomialRing.Polynomial):
                if self.is_ntt ^ other.is_ntt:
                    raise ValueError(
                        f"Both or neither polynomials must be in NTT form before multiplication")
                new_coeffs = [self.sub_mod_q(x, y) for x, y in zip(
                    self.coeffs, other.coeffs)]
            elif isinstance(other, int):
                new_coeffs = self.coeffs.copy()
